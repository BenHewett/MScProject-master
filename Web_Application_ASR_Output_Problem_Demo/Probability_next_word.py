...
def best_candidate(previous_probabilities, subsequent_probabilities, prev_word, sub_word):
    ...
    crossovers = {}
    for word in prev1.keys():
            if word in sub1.keys():
                crossovers[word] = prev1[word]
...
if __name__ == '__main__':
    ...
    bi_sub = form_bigrams_subsequent(corpus='The cat sat on the mat')
    print(bi_sub)

    probs_sub = calculate_probabilities(bigrams=bi_sub)
    print(probs_sub)

    bi_prev = form_bigrams_previous(corpus='The cat sat on the mat')
    print(bi_prev)
